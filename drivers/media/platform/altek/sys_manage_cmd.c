 ...
 *\	 This command support receive more than one package
 *\	 But this function not implement ready. Currently only can work on one
 *\	 package case
 *\param  devdata [In], misp_data
 *\param opcode [In], Operation code
 *\param param [In], CMD param
 ...
 *\return Error code
 */
errcode mast_sys_manage_cmd_get_peripheral_device(
						void *devdata,
						u16 opcode, u8 *param)
{
	/* Error Code*/
	errcode err = ERR_SUCCESS;
	u32 para_size = sizeof(struct isp_cmd_peripheral_info);
	u32 response_para_size = sizeof(
	...
	misp_info("%s - enter", __func__);
	peripheral_info = (struct isp_cmd_peripheral_info *)param;
	misp_info("[dev id]: %d, [RedAddr]: 0x%x, [RegData]: 0x%x",
		peripheral_info->ucDevId,
		peripheral_info->uwRegAddr,
		peripheral_info->uwRegData);
	/* Send command to slave*/
	err = ispctrl_mast_send_cmd_to_slave(devdata, opcode, param, para_size);
	if (err != ERR_SUCCESS)
		goto get_peripheral_device_end;
	/* Get data from slave*/
	err = ispctrl_mast_recv_response_from_slave(devdata,
			param, response_para_size, true);
	info_response = (struct isp_cmd_peripheral_info_response *)param;
	if (err != ERR_SUCCESS) {
		misp_info("%s - err 0x%x", __func__, err);
		goto get_peripheral_device_end;
	}
	misp_info("RegData 0x%x, Response 0x%x",
		info_response->uwRegData, info_response->ucResponse);

	/* return errcode if response represent err */
	if (info_response->ucResponse != ERR_SUCCESS)
		err = info_response->ucResponse;
get_peripheral_device_end:
	return err;
}
/*
 ...
 *\return Error code
 */
errcode mast_sys_manage_cmd_set_peripheral_device(
						void *devdata,
						u16 opcode, u8 *param)
{
	/* Error Code*/
	...
	err = ispctrl_mast_send_cmd_to_slave(devdata, opcode, param, para_size);
	if (err != ERR_SUCCESS)
		goto set_peripheral_device_end;
	/* Get data from slave*/
	err = ispctrl_mast_recv_response_from_slave(devdata,
			param, response_para_size, true);
	info_response = (struct isp_cmd_peripheral_info_response *)param;
 ...
 *\return Error code
 */
errcode mast_sys_manage_cmd_get_binfile_ver(
						void *devdata,
						u16 opcode, u8 *param)
{
	/* Error Code*/
	errcode err = ERR_SUCCESS;
	/** Due to cmd packet 64 byte limitation,
	 *\ here only send 54 byte response data,
	 *\lastest item is ulDepth_Ver
	 */
	u32 response_para_size = 54;
	...
	err = ispctrl_mast_send_cmd_to_slave(devdata, opcode,
		NULL, 0);
	if (err != ERR_SUCCESS)
		goto get_binfile_ver_end;

	/* Get data from slave*/
	err = ispctrl_mast_recv_response_from_slave(devdata,
			param, response_para_size, true);

	info_response = (struct isp_cmd_binfile_version *)param;
	if (err != ERR_SUCCESS) {
		misp_info("%s - err 0x%x", __func__, err);
		goto get_binfile_ver_end;
	}
/*
 *\misp_info("MainCode_MajVer %d, MainCode_MinVer %d",
 *\	info_response->ulMainCode_MajVer,
 *\	info_response->ulMainCode_MinVer);
 *\
 *\misp_info("ulBootCode_ProductID %d, ulBootCode_Ver %d",
 *\	info_response->ulBootCode_ProductID,
 *\	info_response->ulBootCode_Ver);
 *\
 *\
 *\misp_info("SCTable Project %s, SCTable Type %d,
 *\	SCTable_Ver %d",
 *\	info_response->aucSCTablesTag,
 *\	info_response->ucSCTableType,
 *\	info_response->ucSCTable_Ver);
 *\
 *\misp_info("HDR_Ver %d, IRP0_Ver %d, IRP1_Ver %d,
 *\	Depth_Ver %d",
 *\	info_response->ulHDR_Ver,
 *\	info_response->ulIRP0_Ver,
 *\	info_response->ulIRP1_Ver,
 *\	info_response->ulDepth_Ver);
 */
get_binfile_ver_end:
	return err;
}
 ...
 *\return Error code
 */
errcode mast_sys_manage_cmd_turnon_Ocram7(
						void *devdata,
						u16 opcode, u8 *param)
{
	/* Error Code*/
