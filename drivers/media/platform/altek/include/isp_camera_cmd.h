...
#define ISPCMD_CAMERA_ISPSMARTAECONTROLONOFF    0x3021
#define ISPCMD_CAMERA_SET_GROUP_LED_LEVEL       0x3022
#define ISPCMD_CAMERA_SET_CTRL_LED_SEQUENCE     0x3023
#define ISPCMD_CAMERA_HOST_CTRL_LED_ONOFF 0x03026
#define ISPCMD_CAMERA_EXTRA_SENSOR_SYNC 0x03027
#define ISPCMD_CAMERA_SET_DEPTH_TYPE 0x3028
...
#define ISPCMD_BULK_GET_BINFILE_VER 0x210F
...
#define ISPCMD_SYSTEM_GET_PERIPHERAL_DEVICE	0x0116
#define ISPCMD_SYSTEM_SET_PERIPHERAL_DEVICE	0x0117
#define ISPCMD_SYSTEM_CLEAR_ERRORCODE	0x0118
#define ISPCMD_SYSTEM_GET_BINFILE_VER	0x0119
#define ISPCMD_SYSTEM_OCRAM7_TURNON	0x0120
...
#define ALTEK_PACKDATA_SIZE  1824
#define EEPROM_BUFFER_SIZE			0x4000
#define ALTEK_CALIB_INFO_SIZE       4000
...
/**
 *\@struct ISPCMD_SET_GROUP_LED_LEVEL
 *\@brief ISP cmd for setting group led level
 */
#pragma pack(1)
struct ISPCMD_LED_LEVEL {
	/* 0: Projector 1: Flood */
	u8 ucType;
	/* bit 0: Set Light 0, 1: change Light 0 level */
	/* bit 1: Set Light 1, 1: change Light 1 level */
	u8 ucGroup0IDs;
	/* Light level for Group 0, 0~255 */
	u8 ucGroup0Level;
	/* bit 0: Set Light 0, 1: change Light 0 level */
	/* bit 1: Set Light 1, 1: change Light 1 level */
	u8 ucGroup1IDs;
...
/**
 *\@struct ISPCMD_CTRL_LED_SEQ
 *\@brief ISP cmd for control LED SEQUENCE
 */
#pragma pack(1)
struct ISPCMD_CTRL_LED_SEQ {
	/* 0: disable , 1~12: cycle, 13~255: 12 cycles*/
	u8 ucCycle;
	/* bit 0: Projector, bit 1: Flood*/
	u8 ucCycleLightCtrl[12];
	/**array [0] for 1st frame of Cycle N,
	 *\array[M] (M+1)th frame of Cycle N
	 */
};

/**
 *\@struct ISPCMD_CTRL_GPO
 *\@brief ISP cmd for LED ON/OFF
 */
#pragma  pack(1)
struct ISPCMD_CTRL_GPO {
	/* bit 0 : GPO_0  [ 1: High Level, 0: Low Level ]*/
	/* bit 1 : GPO_1  [ 1: High Level, 0: Low Level ]*/
	/* ....                                          */
	/* bit 10: GPO_10 [ 1: High Level, 0: Low Level ]*/
	u16 uwGPO_HL_BitField;
};
/*basic cmd use structure*/

/**
 *\@struct ISPCMD_DEPTH_TYPE
 *\@brief ISP cmd for Depth type
 */
#pragma  pack(1)
struct ISPCMD_DEPTH_TYPE {
	/* 0: Passive Stereo Depth */
	/* 1: Active Stereo Depth  */
	/* 2: Control by Projector */
	u8 ucDepthType;
	/* GroundType,  0x00: Depth wihtout any enhancement */
	/* 0x10: Depth with ground enhancement */
	/* 0x30: Depth with ground enhancement with
	 *\hole fill function
	 */
	/* 0x50: Depth with ground enhancement with
	 *\cliff clear function
	 */
	u8 ucGroundType;
	u8 ucReserved[4];
};
...
/**
 *@struct read_calib_info
 *@brief use on isp calibration info read
 */
#pragma pack(1)
struct read_calib_info {
	u32 total_size;
...
#pragma pack(1)
struct isp_cmd_cycle_trigger_depth_process {
	/* effective length in cycle[],
	 *\Range: 1~16 frames one cycle
	 */
	u8 cycleLen;
...
/**
 *\@struct isp_cmd_peripheral_info
 *\@brief set/get peripheral device info
 */
#pragma pack(1)
struct isp_cmd_peripheral_info {
	// [IN] device id, this should be defined by project
	u8  ucDevId;
	// [IN] device register address
	u16 uwRegAddr;
	// [IN] for write device register data;
	u16 uwRegData;
};
/**
 *\@struct isp_cmd_peripheral_info_response
 *\@brief get peripheral device info response
 */
#pragma pack(1)
struct isp_cmd_peripheral_info_response {
	// [OUT] for read device register data
	u16 uwRegData;
	// [OUT] define component access response
	u8 ucResponse;
};
/**
 *\@struct isp_cmd_binfile_version
 *\@brief get Boot, Main code, SCTable, HDR, IRP0, IRP1, Depth bin file version
 */
#pragma pack(1)
struct isp_cmd_binfile_version {
	// [OUT] BootCode bin file product ID
	u32 ulBootCode_ProductID;
	/** [OUT] BootCode bin file b[31:16]Major,
	 *\b[15:0]Minor version
	 */
	u32 ulBootCode_Ver;
	// [OUT] MainCode bin file product ID
	u32 ulMainCode_ProductID;
	// [OUT] MainCode bin file Major version
	u32 ulMainCode_MajVer;
	// [OUT] MainCode bin file Minor version
	u32 ulMainCode_MinVer;
	// [OUT] SCTable Project tag
	u8 aucSCTablesTag[16];
	// [OUT] SCTable Type. 1: default. 2: AP updated
	u8 ucSCTableType;
	// [OUT] SCTable version
	u8 ucSCTable_Ver;
	/**[OUT] HDR bin file version HDR Major version =
	 *\ulHDR_Ver / 1000(dec)
	 *\HDR Minor version = ulHDR_Ver % 1000(dec)
	 */
	u32 ulHDR_Ver;
	/**[OUT] IRP0 bin file version	IRP0 Major version =
	 *\ulIRP0_Ver / 1000(dec)
	 *\IRP0 Minor version = ulIRP0_Ver % 1000(dec)
	 */
	u32 ulIRP0_Ver;
	/**[OUT] IRP1 bin file version	IRP1 Major version =
	 *\ulIRP1_Ver / 1000(dec)
	 *\IRP1 Minor version = ulIRP1_Ver % 1000(dec)
	 */
	u32 ulIRP1_Ver;
	/**[OUT] Depth bin file version Depth bin build year =
	 *\ulDepth_Ver / 10000(dec)
	 *\Depth bin type = (ulDepth_Ver % 10000) / 1000 (dec)
	 *\Depth bin version = ulDepth_Ver % 1000 (dec)
	 */
	u32 ulDepth_Ver;
	// [OUT] AE tuning bin file Major version
	u32 ulAE_MajorVer;
	// [OUT] AE tuning bin file Minor version
	u32 ulAE_MinorVer;
};
