...
#define MINIISP_DRIVER_VERSION "v1.55"
...
#define I2C_TX_BULK_CTRL_BYTE 0x0A
...
enum MINI_ISP_EVENT {
	...
	MINI_ISP_RCV_LOG_FULL = (1L << 12),/* 0x00001000*/
};
...
enum MINI_SPINOR_ID {
	...
	SPINOR_ID_IRP0 = 4,
	SPINOR_ID_IRP1 = 5,
	SPINOR_ID_DEPTHBIN = 6,
	SPINOR_ID_MAX,
};

enum MINI_ISP_LOAD_CODE_STATUS {
	MINI_ISP_FW_NONE = 0,
	MINI_ISP_BOOT_CODE_LOADING = 1,
	MINI_ISP_BOOT_CODE_FINISH = 2,
	MINI_ISP_MAIN_CODE_LOADING = 3,
	MINI_ISP_MAIN_CODE_FINISH = 4,
};

enum MINI_ISP_TRANS_CLASS {
	MINIISP_GENERAL_OPCODE  = 0,
	MINIISP_BULK_OPCODE     = 1,
	MINIISP_CFG_REG_MEM     = 2,
};

enum MINI_ISP_TRANS_INTF {
	INTF_SPI        = (1 << 0),
	INTF_I2C_TOP    = (1 << 1),
	INTF_I2C_SLAVE  = (1 << 2),
	INTF_CCI        = (1 << 3),
...
/* TRANS_SCENARIO:
 * bit[0:3] is GENERAL_OPCODE_MODE
 * bit[4:7] is BULK_OPCODE_MODE
 * bit[8:11] is CFG_REG_MEM_MODE
 */
#define CFG_TRANS_SCENARIO(G_OPCODE, B_OPCODE, C_REG_MEM) \
	((G_OPCODE)|(B_OPCODE << 4)|(C_REG_MEM << 8))


/** Configure transfer scenario
 *\User can decide GENERAL_OPCODE_MODE,
 *\I2C_BULK_OPCODE_MODE, CFG_REG_MEM_MODE
 *\command which be send by specifiec transfer interface
 */

/* boot from spinor */
/* all use I2c interface */
/**#define MINIISP_TRANS_SCENARIO_SPINOR
 *\CFG_TRANS_SCENARIO(INTF_I2C_TOP,INTF_I2C_TOP, INTF_I2C_SLAVE)
 */

/* all use SPI interface */
#define MINIISP_TRANS_SCENARIO_SPINOR \
	CFG_TRANS_SCENARIO(INTF_SPI, INTF_SPI, INTF_SPI)

/* boot from spi (CFG_REG_MEM_MODE can't use INTF_I2C_SLAVE)*/
/* gerenal command use I2c interface */
/**#define MINIISP_TRANS_SCENARIO
 *\CFG_TRANS_SCENARIO(INTF_I2C_TOP, INTF_SPI, INTF_SPI)
 */
/* all use SPI interface */
#define MINIISP_TRANS_SCENARIO \
	CFG_TRANS_SCENARIO(INTF_SPI, INTF_SPI, INTF_SPI)
...
struct misp_global_variable {
	...
	int irq_num;
	int reset_gpio;
	int intf_status;	// probed interface
	int before_booting;
	int i2c_enable;
	int altek_spi_mode;
	...
	// For avoid reading protocal be interrupt by other transfer action
	struct semaphore transfer_lock;
	// For I2c_top, SPI_A bulk transfer protection
	struct semaphore bulk_lock;
	bool bootfromspinor;
};
...
extern struct misp_data *get_mini_isp_intf(enum
			MINI_ISP_TRANS_CLASS trans_class);
...
extern u32 GET_TRANS_SCEN_INTF(enum MINI_ISP_TRANS_CLASS trans_class);
...
extern errcode mini_isp_leave_cp(u16 opcode, u8 *param);
extern errcode mini_isp_leave_cp_standby(void);
...
extern errcode mini_isp_memory_read_then_write_file(u32 start_addr,
...
extern u32 mini_isp_memory_read_shortlen(u32 start_addr,
		u32 *read_buffer);
...
extern ssize_t mini_isp_register_conti_write(u32 reg_addr,
		u8 *write_buffer, u32 write_len);
extern ssize_t mini_isp_memory_write(u32 memory_addr, u8 *write_buffer,
				u32 write_len);
...
extern void rx_data_debug(const void *buf, int data_len, int dbg_len);
extern u32 mini_isp_enable_irq(void);
extern u32 mini_isp_disable_irq(void);
extern void mini_isp_en_fsm(bool a_bEn);
extern bool mini_isp_is_en_fsm(void);

/*miniisp_customer_define.c*/
extern int mini_isp_power_open(void);
extern int mini_isp_power_close(void);
...
extern struct misp_data *get_mini_isp_intf_i2c_top_data(void);
extern struct misp_data *get_mini_isp_intf_i2c_slave_data(void);

extern u8 g_load_code_status;
extern u16 g_fw_version_before_point;
extern u16 g_fw_version_after_point;
extern char g_fw_build_by[];
extern char g_fw_project_name[];
extern u32 g_sc_build_date;
