...
#define MAGIC_NUMBER_ADDR 0xffef0008
...
#define DEPTH_FLOWMGR_INFO_VER 0x11
struct DEPTHFLOWMGR_DEBUG_INFO {
	/* General info */
	bool ValidFlag;
	u8 Ver;
	u8 ALDU_MainVer;
	...
	struct {
		...
		u32 DepthProcCnt;
		u8  FeatureFlag;
		bool InvRECT_BypassLDC;
		bool PerformanceFlag;
		bool NormalKernalSizeMatch;
		bool GroundKernalSizeMatch;
		u8 NormalKernelSizeIdx;

		u8 GroundKernelSizeIdx;

		bool DepthDPHFDynaChgIteration;
		u8 DepthDPHFIteration;
		bool IsEnable_SolA;
		bool IsEnable_SolB;
		bool IsEnable_RemappingLUT;
		u32 SolAandRemapping_LUT_Addr;
		u32 SolBLUT_Addr;
		u32 RemappingLUT_Addr;
		bool IsEn_SolB2_DispThreshold;
		u32 SolB2_DispThreshold;
		bool IsEn_SolD_NoiseFilter;
		bool IsEn_SolD_ColJump;
		u32 DepthProcTime_max;
		u32 SolATimeus_max;
		u32 SolBTimeus_max;
		u32 SolDTimeus_max;
		bool IsDepthBin_UseDefault;
		bool IsEn_Mixed_SolB2SolDCJ;
		bool IsEn_SolE_Blending;
		bool bIsEn_UnpackDisp;
		u32 udDepthFrameRate;
	} tDepthFlowDebugInfo;

	/* Packdata */
	struct {
		bool ValidDepthPacadataDebugInfo;
		bool ValidPackdata;
		bool ValidDisparityToDistanceTable;
		bool HighDistortionRate;
	...
	/* Detph utility */
	struct {
		bool ValidDepthUtiDebugInfo;
		bool ValidBlendingTable;
		/* Distance */
		u16 DistanceVal[3];
...
enum DEPTHPACKDATA_SRC {
	DEPTHPACKDATA_SRC_NULL,
	/* receive packdata via command */
	DEPTHPACKDATA_SRC_CMD,
	/* receive packdata from OTP, EEPROM (ISP read via I2C) */
	DEPTHPACKDATA_SRC_OTP,
};

enum DEPTHBLENDING_SRC {
	DEPTHBLENDING_SRC_NULL,
	/* AP send blending table */
	DEPTHBLENDING_SRC_CMD,
	/* ISP calculate blending table */
	DEPTHBLENDING_SRC_ISP,
};

struct CAPFLOWMGR_META_COMMON {
	u16 MetaDataVer;
	u16 SCID;
	u8  RxSwap;
	u8  TxSwap;
...
/* For Metadata, check if it will be used in SK1 */
struct CAPFLOWMGR_SHADING_DEBUG_INFO {
	u16 SHDTableW;
	u16 SHDTableH;
...
struct CAPFLOWMGR_META_SWDEBUG {
	...
	u16 DepthInWidth; /* 2 */
	u16 DepthInHeight; /* 2 */
	struct CAPFLOWMGR_SHADING_DEBUG_INFO tPipeSHDInfo[2]; /* 32 */
	struct DEPTHPACKDATA_WOI_INFO tPipeDepthCaliWOI[2]; /* 16 */
	u64  SysTimeus;   /* 8 */
	struct CAPFLOWMGR_PROJECTOR_STATUS tProjectorStatus[8]; /* 16 */
	struct CAPFLOWMGR_GPIO_DEV tGPIO_Device[2]; /* 2 */
	u8 Resv1[6];
	u64  IDD0_ISRTimeus;
	u64  IDD1_ISRTimeus;
	u8 aucResv2[16];
 ...
 *\@brief Depth paramters
 */
struct CAPFLOWMGR_DEPTH_WOI_INFO {
	u16  OriginalWidth;
	u16  OriginalHeight;
...
/* For Metadata, check if it will be used in SK1 */
struct CAPFLOWMGR_META_IQDEBUG {
	u32 Qmerge_Ver;
	u32 audTool_Ver[3];
	u32 audTuning_Ver[3];
	u8  aucVerifyDebug[2];
 ...
 */
struct CAPFLOWMGR_METADATA {
	struct CAPFLOWMGR_META_COMMON tCommonInfo;
	struct CAPFLOWMGR_META_PIPE tPipe0Info;
	struct CAPFLOWMGR_META_PIPE tPipe1Info;
	struct CAPFLOWMGR_META_DEPTH tDpethInfo;
	struct CAPFLOWMGR_META_HDR tHDRInfo;
	u8 aucMetaData_reserved_0[12];
	struct CAPFLOWMGR_META_SWDEBUG tSWDebugInfo;
	struct CAPFLOWMGR_META_IQDEBUG tIQDebugInfo;
	u8 aucMetaData_reserved_1[920];
	u32 audHDRAEHistogram_Short[260];
	u32 audHDRAEHistogram_Long[260];
};

enum E_FEC_SENSOR_MODE {
...
struct FEC_SENSOR_REAL_INFO {
	...
	enum E_FEC_SENSOR_MODE SensorMode; /* see E_FEC_SENSOR_MODE */
};
...
struct PROJECTOR_INFO {
	...
	/**error occurs, and record temperature
	 *\from thermal sensor
	 */
	u16 Temperature;
