...
static DEVICE_ATTR(mini_isp_config, 0644, mini_isp_config_show,
			...
			u32 tx_len, u8 *rx_buf, u32 rx_len)
{
	...
	if (tx_len + rx_len <= 32) {
		status = spi_write_then_read(devdata->cfg.spi,
	...
	spi_message_init(&m);
	spi_message_add_tail(&t, &m);
	status = spi_sync(devdata->cfg.spi, &m);
...
static int mini_isp_intf_spi_probe(struct spi_device *spi)
{
	...
	set_mini_isp_data(misp_intf_spi_data, INTF_SPI);
