...
int mini_isp_get_chip_id(void)
{
	...
	misp_info("[miniISP]Get Chip ID %x %x %x %x",
...
void mini_isp_register_write(u32 reg_addr, u32 reg_new_value)
{
	...
	devdata->intf_fn->write(devdata, devdata->tx_buf, NULL, 9);
	/* dev_global_variable->before_booting = 0; */
}

ssize_t mini_isp_register_conti_write(u32 reg_addr,
	u8 *write_buffer, u32 write_len)
{
	u8 *send_buffer = NULL;
	u8 ctrlbyte = CTRL_BYTE_REGWR;
	u32 wt_len = 0;
	u32 remain_size = 0;
	u32 one_size = 0;
	u32 write_addr = reg_addr;
	ssize_t rtn = ERR_SUCCESS;
	...
	if (GET_TRANS_SCEN_INTF(MINIISP_CFG_REG_MEM) !=
		INTF_I2C_SLAVE) {
		rtn = -EINVAL;
		misp_info("%s:L%d Error Only I2c_slave register conitnue write",
			__func__, __LINE__);
		return rtn;
	}

	send_buffer = kzalloc(EMODE_TXCMD_LEN + write_len,
		GFP_KERNEL);
	if (send_buffer == NULL) {
		rtn = -ENOMEM;
		misp_err("%s, L:%d Allocate memory failed.",
			__func__, __LINE__);
		return rtn;
	}

	wt_len = 0; /* writed size clear to zero */
	for (remain_size = write_len; remain_size > 0;
		remain_size -= one_size) {

		if (remain_size > I2C_TX_BULK_SIZE)
			one_size = I2C_TX_BULK_SIZE;
		else
			one_size = remain_size;

		/* update send buffer context */
		memcpy(send_buffer, &ctrlbyte, 1);
		memcpy(send_buffer + 1, &write_addr, 4);
		memcpy(send_buffer + 5, write_buffer + wt_len, one_size);

		devdata->intf_fn->write(devdata,
					send_buffer,
					NULL,
					one_size + EMODE_TXCMD_LEN);
...
void mini_isp_e_to_a(void)
{
	...
	dev_global_variable = get_mini_isp_global_variable();
	if (!((dev_global_variable->intf_status & INTF_SPI) &&
		(GET_TRANS_SCEN_INTF(MINIISP_CFG_REG_MEM) ==
		INTF_SPI) && (dev_global_variable->altek_spi_mode ==
		ALTEK_SPI_MODE_E))) {
		misp_info("%s, Block e2a", __func__);
		return;
	}
...
void mini_isp_a_to_e(void)
{
	...
	dev_global_variable = get_mini_isp_global_variable();
	if (!((dev_global_variable->intf_status & INTF_SPI) &&
		(GET_TRANS_SCEN_INTF(MINIISP_CFG_REG_MEM) == INTF_SPI) &&
		(dev_global_variable->altek_spi_mode == ALTEK_SPI_MODE_A))) {

		misp_info("%s, L:%d Block a2e", __func__, __LINE__);
		return;
	} else if (g_load_code_status != MINI_ISP_MAIN_CODE_LOADING &&
	...
	devdata = get_mini_isp_intf_spi();
	send_buffer = devdata->tx_buf;

	memcpy(send_buffer, &ctrlbyte, 1);
	devdata->intf_fn->write(devdata, devdata->tx_buf, NULL, 1);
	dev_global_variable->altek_spi_mode = ALTEK_SPI_MODE_E;
	misp_info("mini_isp_a_to_e");
}
...
errcode mini_isp_leave_cp(u16 opcode, u8 *param)
{
	int err = 0;
	struct misp_global_variable *dev_global_variable;
	...
	/* Set Sensor Mode */
	err = ispctrl_if_mast_execute_cmd(opcode, param);
	return err;
}

errcode mini_isp_leave_cp_standby(void)
{
	int err = 0;
	struct misp_global_variable *dev_global_variable;
...
int mini_isp_pure_bypass(u16 mini_isp_mode)
{
	...
	if (mini_isp_device != NULL && byass_setting_file_location != NULL) {
		file_name = strrchr(byass_setting_file_location, '/');
	...
	while (CurBufPos < file_total_size) {
		...
		} else if (sscanf(linebuf, "sl %d", &sleepTime) == 1) {
...
u32 mini_isp_register_read(u32 reg_addr, u32 *reg_value)
{
	...
	if (SPI_SHORT_LEN_MODE && SPI_SHORT_LEN_MODE_READ_ENABLE &&
		...
		recv_buffer = (u8 *)&recv_buffer_value[0];
...
ssize_t mini_isp_memory_write(u32 memory_addr, u8 *write_buffer, u32 write_len)
{
	u8 *send_buffer = NULL;
	u8 ctrlbyte = CTRL_BYTE_MEMWR;
	u32 write_addr = memory_addr;
	...
	u32 one_size = 0;
	u8 *wt_buffer;
	u32 memwrite_blksize = SPI_TX_BULK_SIZE;
	u32 rtn = ERR_SUCCESS;
	...
	if ((GET_TRANS_SCEN_INTF(MINIISP_CFG_REG_MEM)
		...
		misp_err("%s, L:%d err! I2C_slave mem write len must be 4 alignment",
			 __func__, __LINE__);
		return rtn;
	}

	if (SPI_SHORT_LEN_MODE && SPI_SHORT_LEN_MODE_WRITE_ENABLE) {
		send_buffer = kzalloc(EMODE_TXCMD_LEN + 4, GFP_KERNEL);
		if (send_buffer == NULL) {
			rtn = -ENOMEM;
			misp_err("%s, Alloc mem failed", __func__);
			return rtn;
		}
		...
		if (GET_TRANS_SCEN_INTF(MINIISP_CFG_REG_MEM)
				== INTF_SPI)
			memwrite_blksize = 6000;
		else if (GET_TRANS_SCEN_INTF(MINIISP_CFG_REG_MEM)
				== INTF_I2C_SLAVE)
			memwrite_blksize = I2C_TX_BULK_SIZE;
		else
			memwrite_blksize = I2C_TX_BULK_SIZE;

		send_buffer = kzalloc(EMODE_TXCMD_LEN +
			memwrite_blksize, GFP_KERNEL);
		if (send_buffer == NULL) {
			rtn = -ENOMEM;
			misp_err("%s, L:%d Allocate memory failed.",
				__func__, __LINE__);
		...
		wt_len = 0; /* writed size clear to zero */
		for (remain_size = write_len;
			remain_size > 0;
			remain_size -= one_size) {

			if (remain_size > memwrite_blksize)
				one_size = memwrite_blksize;
			else
				one_size = remain_size;
			...
			memcpy(send_buffer, &ctrlbyte, 1);
			memcpy(send_buffer + 1, &write_addr, 4);
			memcpy(send_buffer + 5, write_buffer +
				wt_len, one_size);

			/**
			 *\misp_info("remain: %d, one_size: %d,
			 *\write_addr: 0x%x",
			 *\remain_size, one_size, write_addr);
			 */
			devdata->intf_fn->write(
					devdata,
					send_buffer,
...
u32 mini_isp_memory_read_then_write_file(u32 start_addr,
	u32 len, char *dest_path, char *file_name)
{
	u32 retval = ERR_SUCCESS;
	u8 *dump_memory   = NULL;
	u8 filename[128];
 ...
 *\param width      [In], image width (byte),
 *\it can't exceed 60000bytes.
		...
		char *dest_path, char *file_name)
{
	...
	for (loopidx = 0, remain_size = ouput_size; remain_size > 0;
		...
		retval = mini_isp_memory_read(dump_addr,
...
u32 mini_isp_memory_read(u32 start_addr, u8 *read_buffer, u32 len)
{
	...
	u32 ouput_size;
	u32 io_size, remain_size, one_size, recv_buffer_size;
	u32 send_buffer_size;
	u32 rx_dummy_len;
	u8 ctrlbyte = CTRL_BYTE_MEMRD;
	u32 len_align;
	u32 memread_blksize = I2C_TX_BULK_SIZE;
	...
	if (SPI_SHORT_LEN_MODE && SPI_SHORT_LEN_MODE_READ_ENABLE &&
		...
		if (GET_TRANS_SCEN_INTF(MINIISP_CFG_REG_MEM) == INTF_SPI)
			memread_blksize = 6000;
		else if (GET_TRANS_SCEN_INTF(MINIISP_CFG_REG_MEM)
				== INTF_I2C_SLAVE)
			// I2c rx buffer excess 6000byte may fail
			memread_blksize = I2C_TX_BULK_SIZE;
		else
			memread_blksize = I2C_TX_BULK_SIZE;
		...
		recv_buffer_size = EMODE_TXCMD_LEN + rx_dummy_len
			+ memread_blksize;

		/* read memread_blksize bytes at a time;*/
		io_size = send_buffer_size + recv_buffer_size;
		io_buffer = kzalloc(io_size, GFP_KERNEL|GFP_DMA);
		if (!io_buffer) {
			misp_err("%s Allocate memory failed.", __func__);
			goto allocate_memory_fail;
		}
		...
		for (remain_size = ouput_size; remain_size > 0;
			...
			one_size = (remain_size > memread_blksize) ?
			memread_blksize : remain_size;
		...
		u32 total_size, u32 block_size)
{
	...
	dev_global_variable = get_mini_isp_global_variable();
	down(&dev_global_variable->bulk_lock);

	send_buffer = io_buffer;
	send_buffer[0] = ctrlbyte;
	recv_buffer = io_buffer + 1;

	/* only spi interface need send ctrl byte */
	if (devdata == get_mini_isp_intf_spi()) {
		status = devdata->intf_fn->read(
							(void *)devdata,
							send_buffer,
							1,
...
G_EXIT:
	...
	up(&dev_global_variable->bulk_lock);
...
int mini_isp_get_altek_status(void *devdata, u32 *altek_status)
{
	...
	mini_isp_register_read(INTERRUPT_STATUS_REGISTER_ADDR,
		altek_status);
...
extern irqreturn_t mini_isp_irq(int irq, void *handle)
{
	...
	errcode = mini_isp_get_altek_status(devdata,
		&altek_event_state);
	misp_info("%s - read INT register: %#x",
	...
	if (errcode == ERR_SUCCESS) {
		...
		/* common log full event*/
		if (altek_event_state & COMMON_LOG_FULL)
			event = event | MINI_ISP_RCV_LOG_FULL;
...
int mini_isp_wait_for_event(u32 e)
{
	...
	chk_event = e | (MINI_ISP_RCV_ERROR | MINI_ISP_RCV_ERROR2);

	do {
		state = wait_event_interruptible(WAITQ, event & chk_event);
	} while (state == (-ERESTARTSYS));

	current_event = event;
	event = (~chk_event) & event;
...
#endif
	misp_info("%s - leaving. event: %#x", __func__, current_event);
	return state;
}
...
u32 mini_isp_check_rx_dummy(u8 **recv_buffer, u32 rx_dummy_len)
{
	...
	for (col_idx = 0; col_idx < ((rx_dummy_len+1)/16); col_idx++) {
		misp_info("%02X %02X %02X %02X %02X %02X %02X %02X %02X \
			%02X %02X %02X %02X %02X %02X %02X",
			(u32)*(ptest), (u32)*(ptest+1), (u32)*(ptest+2),
			(u32)*(ptest+3), (u32)*(ptest+4), (u32)*(ptest+5),
			(u32)*(ptest+6), (u32)*(ptest+7), (u32)*(ptest+8),
			(u32)*(ptest+9),  (u32)*(ptest+10), (u32)*(ptest+11),
			(u32)*(ptest+12), (u32)*(ptest+13), (u32)*(ptest+14),
			(u32)*(ptest+15));
	...
	col_idx -= 1;
	for (row_idx = 0; row_idx < (rx_dummy_len+1)-
		(col_idx*16); row_idx++) {
...
#if EN_SPIE_DEBUG
	misp_info("%s, available byte: 0x%x",
		__func__, (u32)(**recv_buffer));
#endif
	return ret;
}
...
#endif
	} else if ((dev_global_variable->intf_status & INTF_I2C_TOP) &&
		(dev_global_variable->intf_status & INTF_I2C_SLAVE) &&
		(GET_TRANS_SCEN_INTF(MINIISP_CFG_REG_MEM) == INTF_I2C_SLAVE))
		rx_dummy_byte = 0;
	else
		rx_dummy_byte = 0;
...
void rx_data_debug(const void *buf, int data_len, int dbg_len)
{
	int len = 0, pos = 0;
...
u32 mini_isp_enable_irq(void)
{
	...
	if (IRQ_GPIO != NULL) {
		...
		enable_irq(dev_global_variable->irq_num);
	}

	return ret;
}

u32 mini_isp_disable_irq(void)
{
	errcode ret = ERR_SUCCESS;
	struct misp_global_variable *dev_global_variable;
	...
	if (IRQ_GPIO != NULL) {
		misp_info("%s", __func__);
		disable_irq(dev_global_variable->irq_num);
	}
	return ret;
}

void mini_isp_en_fsm(bool a_bEn)
{
	g_bEnFSM = a_bEn;
	if (g_bEnFSM)
		misp_info("%s FSM Enable", __func__);
	else
		misp_info("%s FSM Disable", __func__);
}

bool mini_isp_is_en_fsm(void)
{
	struct misp_global_variable *dev_global_variable;
	...
	if (g_bEnFSM == false)
		return false;
	else if (dev_global_variable->bootfromspinor)
		return false;
	else
		return true;
}
