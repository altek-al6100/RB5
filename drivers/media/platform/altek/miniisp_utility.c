...
#define PACKDATA_OFFSET \
	((u64)&((struct DEPTHFLOWMGR_DEBUG_INFO *)0)-> \
	tDepthPackdataDebugInfo.PackdataAddr)



#define LOG_BUF_SIZE (200*1024)

struct GPIO g_atGPIO[] = {
...
#define GPIO_NUMBER		(sizeof(g_atGPIO)/sizeof(struct GPIO))
	...
	struct irp_and_depth_information *info)
{
	...
	if (fov_mode == 1)
		...
		misp_info("%s:depth_image_addr:%u, depth_width:%u, depth_height:%u",
		__func__,
		info->depth_image_address, info->depth_width,
		info->depth_height);
		misp_info("irp_fmt:%u, irp_img_addr:%u,irp_w:%u, irp_h:%u, fov_m:%u",
		info->irp_format,
...
int mini_isp_debug_dump_img(void)
{
	...
	/* Step 1 */
	*(u32 *)write_buffer = 1;
	mini_isp_memory_write(0x10, write_buffer, 4);
...
int mini_isp_debug_depth_rect_combo_dump(u8 is_ground_mode)
{
	...
	for (loopIdx = 0; loopIdx < loopNum; loopIdx++) {
		...
		*(u32 *)write_buffer = 1;
		...
		/* step 2 */
		*(u32 *)write_buffer = 1;
		mini_isp_memory_write(0x20, write_buffer, 4);
		msleep(100);
		...
		*(u32 *)write_buffer = 2;
		...
		errcode = mini_isp_create_directory(filepath);
		mini_isp_chip_base_define_module_reg_dump(filepath,
			"rectify_a_0");
		...
		misp_info("step3 m [ref_w,ref_h]: %d ,%d",
		...
		*(u32 *)write_buffer = 2;
		...
		snprintf(filename, 80, "%s_%s", (loopIdx == 0)?"n":"g",
			"step4_mainRect_img_8bit.raw");
		errcode = mini_isp_woi_memory_read_then_write_file(imgAddr,
			ref_imgLinePixelOffset, ref_img_width, ref_img_height,
				filepath, filename);
		...
		*(u32 *)write_buffer = 3;
		...
		errcode = mini_isp_woi_memory_read_then_write_file(0x20100000,
			1024*2, ref_img_width*2, ref_img_height,
			filepath, filename);
		...
		*(u32 *)write_buffer = 4;
		...
		/* dump reg */
		snprintf(filepath, 80,
		"%s/depth_combo_dump/%s_step6_dp_top_a_0/",
			MINIISP_INFO_DUMPLOCATION, (loopIdx == 0)?"n":"g");
		errcode = mini_isp_create_directory(filepath);
		mini_isp_chip_base_define_module_reg_dump(filepath,
			"dp_top_a_0");
		...
		*(u32 *)write_buffer = 5;
		...
		/* dump reg */
		snprintf(filepath, 80,
			"%s/depth_combo_dump/%s_step7_rectify_b_0/",
			MINIISP_INFO_DUMPLOCATION, (loopIdx == 0)?"n":"g");
		errcode = mini_isp_create_directory(filepath);
		mini_isp_chip_base_define_module_reg_dump(filepath,
			"rectify_b_0");
		...
		errcode = mini_isp_memory_read_then_write_file(imgAddr,
			(ref_img_width * ref_img_height * 10 / 8),
			filepath, filename);
	...
	if (is_ground_mode) {
		*(u32 *)write_buffer = 6;
		...
		/* dump reg */
		snprintf(filepath, 80,
			"%s/depth_combo_dump/step9_rectify_b_0/",
			MINIISP_INFO_DUMPLOCATION);
		errcode = mini_isp_create_directory(filepath);
		mini_isp_chip_base_define_module_reg_dump(filepath,
			"rectify_b_0");
		...
		errcode = mini_isp_woi_memory_read_then_write_file(0x20100000,
			1024*2, ref_img_width*2,
			ref_img_height, filepath,
			"g_step9_blending_img_10unpack.raw");

		misp_info("[Step9] InvRect W,H: %d, %d",
			ref_img_width, ref_img_height);
		snprintf(filepath, 80, "%s/depth_combo_dump/",
			MINIISP_INFO_DUMPLOCATION);
		errcode = mini_isp_create_directory(filepath);
		...
		errcode = mini_isp_memory_read_then_write_file(0x20715400,
			(ref_img_width * ref_img_height * 10 / 8), filepath,
			"g_step9_InvRect_img_10pack.raw");
	}
...
errcode mini_isp_debug_packdata_dump(void)
{
	...
	mini_isp_memory_read(DEBUG_INFO_DEPTH_FLOW_INFO_ADDR,
		(u8 *)&packdebuginfo_addr, 4);
	if (packdebuginfo_addr != 0) {
		mini_isp_memory_read(packdebuginfo_addr +
			(u32) PACKDATA_OFFSET, (u8 *)&packdata_addr, 4);
	} else{
...
errcode mini_isp_debug_metadata_dump(void)
{
	...
	if (metadata_addr != 0) {
		...
		metadata_addr, 4*1024, filepath, "Dump_MetaData.bin");
	} else
		misp_info("%s, addr err", __func__);
...
static void mini_isp_debug_depth_info_parser(struct DEPTHFLOWMGR_DEBUG_INFO
	*ptDepthFlowInfo)
{
	u32 modeid;
	u32 refImg1_w, refImg1_h;
	u32 refImg2_w, refImg2_h;
	u32 refImg3_w, refImg3_h;
	...
	if (ptDepthFlowInfo->Ver != DEPTH_FLOWMGR_INFO_VER) {
		...
		misp_info("Linux depth debug ver: 0x%X",
			DEPTH_FLOWMGR_INFO_VER);
		return;
	}
	...
	misp_info("Depth debug version: 0x%X", ptDepthFlowInfo->Ver);
	misp_info("alDU ver: %d.%d", ptDepthFlowInfo->ALDU_MainVer,
		ptDepthFlowInfo->ALDU_SubVer);
	misp_info("=== DepthFlowInfo: General ===");


	if (ptDepthFlowInfo->tDepthFlowDebugInfo.ValidDepthFlowDebugInfo
		== false) {
		...
		misp_info("Mode ID:%d.(error mode id out of range %d~%d)",
	...
	if (refImg2_Idx != refImg1_Idx)
		misp_info("ref1 Img resolution is %s,but ref2 Img resolution is %s.",
		ResStr[refImg1_Idx],
		ResStr[refImg2_Idx]);

	if (refImg2_w > refImg1_w)
		misp_info("(Warning!. ref2 Img width %d > ref1 Img width %d.",
		refImg2_w, refImg1_w);

	if (refImg2_h > refImg1_h)
		misp_info("(Warning!. ref2 Img height %d > ref1 Img height %d.",
		refImg2_h, refImg1_h);
	...
	if (refImg3_Idx != RESOLUTION_OTHER)
		misp_info("ref3 Img Resolution: %dx%d. (%s)", refImg3_w,
		refImg3_h, ResStr[refImg3_Idx]);
	else {
		/* mutiple 100 for division */
		tmp = refImg3_h * 16 * 100 / refImg3_w;
		misp_info("ref3 Img Resolution: %dx%d. (16:%d.%d)",
			refImg3_w, refImg3_h,
			tmp/100, tmp%100);
	}

	/* kernel size ratio */
	KernelSizeRatio = ptDepthFlowInfo->
		tDepthFlowDebugInfo.DepthImgKernelSizeRatio;
	if (KernelSizeRatio > KernelSizeRatio_max)
		misp_info("Kernel Size Ratio: %d.(err ration output of range %d~%d).",
		KernelSizeRatio, KernelSizeRatio_min,
		KernelSizeRatio_max);
	else
		misp_info("Kernel Size Ratio: %d. (valid range %d~%d).",
		KernelSizeRatio, KernelSizeRatio_min, KernelSizeRatio_max);

	/* Depth type */
	DepthType = ptDepthFlowInfo->tDepthFlowDebugInfo.DepthType;
	misp_info("Depth Type: %d. (%s)",
	DepthType, (DepthType > 0) ? "Active" : "Passive");

	/* Ground mode */
	GroundMode = ptDepthFlowInfo->tDepthFlowDebugInfo.DepthGroundType;
	switch (GroundMode) {
	case 0:
		misp_info("GND Type: 0 (Normal mode only).");
	break;
	case 0x10:
		misp_info("GND Type: 0x%X. (GND Enhance).",
			GroundMode);
	break;
	case 0x30:
		misp_info("GND Type: 0x%X. (GND Enhance + Advance Hold Fill).",
		GroundMode);
	break;
	case 0x50:
		misp_info("GND Type: 0x%X. (GND Enhance + Anti-Cliff).",
		GroundMode);
	break;
	case 0x70:
		misp_info("GND Type: 0x%X.GND Enh + Advance Hold Fill + Anti-Cliff",
		GroundMode);
	break;
	default:
		misp_info("GND Type: 0x%X. (Unkown feature).", GroundMode);
	break;
	}

	/* Processing time */
	if (ptDepthFlowInfo->tDepthFlowDebugInfo.DephtProcTime != 0) {
		tmp = ptDepthFlowInfo->
			tDepthFlowDebugInfo.DephtProcTime*100/1000;
		misp_info("Processing Time: %d.%d ms.", tmp/100, tmp%100);
		tmp = 1000000*100/ptDepthFlowInfo->
			tDepthFlowDebugInfo.DephtProcTime;
	...
	tmp = ptDepthFlowInfo->
		tDepthFlowDebugInfo.BlendingProcTime*100 / 1000;
	misp_info("Blending processing time: %d.%d ms.", tmp/100, tmp%100);

	/* Frame count */
	misp_info("Frame Count: %d.", ptDepthFlowInfo->
		tDepthFlowDebugInfo.DepthProcCnt);

	/* Feature flag: InvRec LDC bypass */
	misp_info("Feature flag: %d.", ptDepthFlowInfo->
		tDepthFlowDebugInfo.FeatureFlag);

	/* InvRECT_BypassLDC */
	misp_info("InvRect bypass LDC: %d.", ptDepthFlowInfo->
		tDepthFlowDebugInfo.InvRECT_BypassLDC);
	...
	/* PerformanceFlag */
	/**[Debug] FALSE: DEPTH_PERFORMANCE_FLAG disable,
	 *\TRUE: DEPTH_PERFORMANCE_FLAG enable
	 */
	misp_info("Performance flag: %d.",
		ptDepthFlowInfo->tDepthFlowDebugInfo.PerformanceFlag);

	/* NormalKernalSizeMatch */
	/**[Debug] Valid when DEPTH_PERFORMANCE_FLAG enable,
	 *\TRUE: Kernel size of normal mode can be extimated by
	 *\clock and Frame rate
	 */
	misp_info("Normal kernel size match flag: %d.",
		ptDepthFlowInfo->tDepthFlowDebugInfo.NormalKernalSizeMatch);

	/* GroundKernalSizeMatch */
	/** [Debug] Valid when DEPTH_PERFORMANCE_FLAG enable,
	 *\TRUE: Kernel size of ground mode can be extimated by
	 *\clock and Frame rate
	 */
	misp_info("Ground kernel size match flag: %d.",
		ptDepthFlowInfo->tDepthFlowDebugInfo.GroundKernalSizeMatch);

	/* NormalKernelSizeIdx */
	/**[Debug] Kernel size idx of normal mode,
	 *\if NormalKernalSizeMatch is False,
	 *\then it means the kernel size could not
	 *\be extimated by clock and frame rate
	 */
	misp_info("Normal kernel size index: %d. (0~15)",
		ptDepthFlowInfo->tDepthFlowDebugInfo.NormalKernelSizeIdx);

	/* GroundKernelSizeIdx */
	/**[Debug] Kernel size idx of ground mode,
	 *\if GroundKernalSizeMatch is False,
	 *\then it means the kernel size could not be
	 *\extimated by clock and frame rate
	 */
	misp_info("Ground kernel size index: %d. (0~15)",
		ptDepthFlowInfo->tDepthFlowDebugInfo.GroundKernelSizeIdx);
DepthFlowDebugInfoEnd:

	misp_info("=== DepthFlowInfo: Utility ===");
	if (ptDepthFlowInfo->tDepthUtiDebugInfo.ValidDepthUtiDebugInfo
		== false) {
	...
	/* ValidBlendingTable */
	ValidBlendingTable = ptDepthFlowInfo->
		tDepthUtiDebugInfo.ValidBlendingTable;
	if (ValidBlendingTable) {
		/* Set blending table source */
		BlendingSrc = ptDepthFlowInfo->
			tDepthUtiDebugInfo.BlendingTableSource;
		if (BlendingSrc == DEPTHBLENDING_SRC_CMD)
			misp_info("Blending table is available: %d.",
			BlendingSrc);
		else if (BlendingSrc == DEPTHBLENDING_SRC_ISP)
			misp_info("Blending table is available: %d.",
			BlendingSrc);
		else
			misp_info("Blending table source from nowhere: %d.",
			BlendingSrc);
	}
	...
	misp_info("Distance: (upper, middle, lower) = (%d, %d, %d)mm.",
		dist[0], dist[1], dist[2]);

	/* Blending table source and size */
	misp_info("Blending table source: %d. (0:None 1:AP 2:ISP)",
		ptDepthFlowInfo->tDepthUtiDebugInfo.BlendingTableSource);
	misp_info("Blending table size: %d.",
		ptDepthFlowInfo->tDepthUtiDebugInfo.BlendingTableSize);

	/* Blending start line */
	misp_info("Blending start line: %d.",
		ptDepthFlowInfo->tDepthUtiDebugInfo.BlendingStartLine);

	/* Valid QMerge */
	ValidQmerge = ptDepthFlowInfo->tDepthUtiDebugInfo.ValidQmerge;
	if (ValidQmerge)
		misp_info("QMerge table updated by AP: %d. (Received)",
		ValidQmerge);
	else
		misp_info("QMerge table updated by AP: %d. (No updated)",
		ValidQmerge);
...
UtilityEnd:
	...
	if (ptDepthFlowInfo->
		tDepthPackdataDebugInfo.ValidDepthPacadataDebugInfo == false) {
		misp_info("Packdata information all invalid.");
		goto PackdataDebugInfoEnd;
	}

	/* check AP download packdata to ISP or not */
	ValidPackdata = ptDepthFlowInfo->
		tDepthPackdataDebugInfo.ValidPackdata;
	if (ValidPackdata)
		misp_info("Valid Packdata=%d. (Packdata download completed.).",
		ValidPackdata);
	else{
		misp_info("Valid Packdata=%d.", ValidPackdata);
		misp_info("Download failed ,qulity and flow may fail.");
	}

	/* Disparity to distance table convert. */
	ValidDisparityToDistanceTable = ptDepthFlowInfo->
		tDepthPackdataDebugInfo.ValidDisparityToDistanceTable;
	if (ValidDisparityToDistanceTable)
		misp_info("Valid Disparity to Distance Table = %d.",
	...
	HighDistortionRateFlag = ptDepthFlowInfo->
		tDepthPackdataDebugInfo.HighDistortionRate;
	if (HighDistortionRateFlag)
		misp_info("High distortion rate flag: %d.",
		HighDistortionRateFlag);
	else {
		misp_info("High distortion rate flag: %d.",
		HighDistortionRateFlag);
		misp_info("high distortion rate flag should be 1.");
	}

	/* Packdata usage index */
	PackdataIdxNormal = ptDepthFlowInfo->
		tDepthPackdataDebugInfo.PackdataNormalIdx;
	PackdataIdxGround = ptDepthFlowInfo->
		tDepthPackdataDebugInfo.PackdataGroundIdx;
	PackdataIdxRGB = ptDepthFlowInfo->
		tDepthPackdataDebugInfo.PackdataRGBIdx;
	if ((PackdataIdxNormal <= 2) && (PackdataIdxNormal <= 2)
			&& (PackdataIdxNormal <= 2))
		misp_info("Packdata index:(normal, ground, RGB) = (%d, %d, %d).",
	...
	/* Global compensation */
	GlobalCompensation = ptDepthFlowInfo->
		tDepthPackdataDebugInfo.AbsoluteGlobalVal;
	if (GlobalCompensation == GlobalCompensation_21 ||
		GlobalCompensation == GlobalCompensation_36)
		misp_info("Compensation value: %d. (valid value is %d or %d)",
			GlobalCompensation, GlobalCompensation_21,
			GlobalCompensation_36);
	else
		misp_info("Compensation value: %d. (it should be %d or %d)",
			GlobalCompensation, GlobalCompensation_21,
			GlobalCompensation_36);

	/* HwCoefficient */
	HwCoefficient = ptDepthFlowInfo->
		tDepthPackdataDebugInfo.HwCoefficient;
	if (HwCoefficient == HwCoefficient_16)
		misp_info("HW coefficient: %d. (valid value is %d)",
		HwCoefficient, HwCoefficient_16);
	else
		misp_info("HW coefficient: %d. (error! it should be %d)",
		HwCoefficient, HwCoefficient_16);

	/* index of high distrotion rate */
	HighDistortionIdx = ptDepthFlowInfo->
		tDepthPackdataDebugInfo.HighDistortionIdx;
	if (HighDistortionIdx <= uwHighDistortionIdx_max)
		misp_info("High distortion index: %d. (valid range 0~5)",
			HighDistortionIdx);
	else
		misp_info("High distortion index: %d. (error! if should be 0~5)",
			HighDistortionIdx);

	/* Packdata source */
	PackdataSrc = ptDepthFlowInfo->
		tDepthPackdataDebugInfo.PackdataSource;
	if (PackdataSrc == DEPTHPACKDATA_SRC_CMD)
		misp_info("Packdata source from AP: %d. (Received)",
			PackdataSrc);
	else if (PackdataSrc == DEPTHPACKDATA_SRC_OTP)
		misp_info("Packdata source from ISP reading OTP: %d.",
			PackdataSrc);
	else
		misp_info("packdata source from nowhere: %d. (fail)",
		PackdataSrc);
	...
	misp_info("MCam WOI (x,y,w,h) = (%d, %d, %d, %d)(FS WxH = %dx%d)",
	...
	Full_w = WOI_w + (WOI_x) * 2;
	Full_h = WOI_h + (WOI_y) * 2;
	misp_info("SCam WOI (x,y,w,h) = (%d, %d, %d, %d)(FS WxH = %dx%d)",
		WOI_x, WOI_y, WOI_w, WOI_h, Full_w, Full_h);
	...
	misp_info("IntrinsicK_Main[9] = %d, %d, %d, %d, %d, %d, %d, %d, %d.",
		ptDepthFlowInfo->
			tDepthPackdataDebugInfo.IntrinsicK_Main[0],
		ptDepthFlowInfo->
			tDepthPackdataDebugInfo.IntrinsicK_Main[1],
		ptDepthFlowInfo->
			tDepthPackdataDebugInfo.IntrinsicK_Main[2],
		ptDepthFlowInfo->
			tDepthPackdataDebugInfo.IntrinsicK_Main[3],
		ptDepthFlowInfo->
			tDepthPackdataDebugInfo.IntrinsicK_Main[4],
		ptDepthFlowInfo->
			tDepthPackdataDebugInfo.IntrinsicK_Main[5],
		ptDepthFlowInfo->
			tDepthPackdataDebugInfo.IntrinsicK_Main[6],
		ptDepthFlowInfo->
			tDepthPackdataDebugInfo.IntrinsicK_Main[7],
		ptDepthFlowInfo->
			tDepthPackdataDebugInfo.IntrinsicK_Main[8]);

	/* check ALDU errcode */
	if (ptDepthFlowInfo->tDepthPackdataDebugInfo.AlDUErrCode == 0)
		misp_info("alDU operation error code: 0x%X. (success)",
			ptDepthFlowInfo->tDepthPackdataDebugInfo.AlDUErrCode);
	else {
		misp_info("alDU operation fail, error code: 0x%X.",
			ptDepthFlowInfo->tDepthPackdataDebugInfo.AlDUErrCode);
	...
	if (ptDepthFlowInfo->tDepthPackdataDebugInfo.ConvertMatch == 0)
		misp_info("Doen't match alDU convert process time: %d.",
			ptDepthFlowInfo->tDepthPackdataDebugInfo.ConvertMatch);
	else
		misp_info("Match alDU convert process time: %d.",
			ptDepthFlowInfo->tDepthPackdataDebugInfo.ConvertMatch);

	misp_info("Packdata convert count: %d.",
		ptDepthFlowInfo->tDepthPackdataDebugInfo.ConvertCount);
	misp_info("Normal mode rectify QP pingpong index: %d.",
		ptDepthFlowInfo->
			tDepthPackdataDebugInfo.NormalRectQPReadPingPongIdx);

	if (ptDepthFlowInfo->tDepthFlowDebugInfo.DepthGroundType == 0) {
		if (ptDepthFlowInfo->
			tDepthPackdataDebugInfo.NormalRectQPReadPingPongIdx !=
			((ptDepthFlowInfo->
				tDepthPackdataDebugInfo.ConvertCount + 1) % 2))
...
errcode mini_isp_debug_depth_info(void)
{
	...
	struct DEPTHFLOWMGR_DEBUG_INFO tDepthFlowInfo = {0};
	...
	mini_isp_memory_read(depthinfo_addr, (u8 *)&tDepthFlowInfo,
		sizeof(struct DEPTHFLOWMGR_DEBUG_INFO));
	...
	if (packdata_buffer != NULL) {
		mini_isp_memory_read(packdata_addr,
			packdata_buffer, PACKDATA_SIZE);
		PackdataCheckSum = CalculatePackdataCheckSum(packdata_buffer);
		if (PackdataCheckSum ==
			tDepthFlowInfo.tDepthPackdataDebugInfo.PackdataChkSum)
			misp_info("Packdata check sum: 0x%X. (equal tool).",
			tDepthFlowInfo.tDepthPackdataDebugInfo.PackdataChkSum);
		else
			misp_info("Packdata check fail: ISP:0x%X. Tool:0x%X.",
			tDepthFlowInfo.tDepthPackdataDebugInfo.PackdataChkSum,
			PackdataCheckSum);
...
static void mini_isp_debug_metadata_parser(struct CAPFLOWMGR_METADATA
	*ptMetadata)
{
	...
	misp_info("Metadata version: %d.", ptMetadata->
		tCommonInfo.MetaDataVer);
	misp_info("SCID: %d.", ptMetadata->tCommonInfo.SCID);
	misp_info("Swap(Rx,Tx): (%d,%d).", ptMetadata->
		tCommonInfo.RxSwap, ptMetadata->tCommonInfo.TxSwap);
	misp_info("SensorType: %d.", ptMetadata->tCommonInfo.SensorType);
	misp_info("HDR (PPIdx, LPPw, LPPh, RPPw, RPPh): (%d,%d,%d,%d,%d).",
		ptMetadata->tCommonInfo.HDRPPPipeIdx,
		ptMetadata->tCommonInfo.LPPWidth,
		ptMetadata->tCommonInfo.LPPHeight,
		ptMetadata->tCommonInfo.RPPWidth,
	...
	misp_info("SrcImg (width, height, ColorOrder): (%d, %d, %d).",
		ptMetadata->tPipe0Info.SrcImgWidth,
		ptMetadata->tPipe0Info.SrcImgHeight,
		ptMetadata->tPipe0Info.ColorOrder);
	misp_info("(ExpTime, wBV): (%d, %d).",
		ptMetadata->tPipe0Info.ExpTime, ptMetadata->tPipe0Info.BV);
	misp_info("(ISO, ADGain): (%d, %d).",
		ptMetadata->tPipe0Info.ISO, ptMetadata->tPipe0Info.AD_Gain);
	misp_info("AWBGain(R,G,B): (%d, %d, %d).",
		ptMetadata->tPipe0Info.AWB_RGain,
		ptMetadata->tPipe0Info.AWB_GGain,
		ptMetadata->tPipe0Info.AWB_BGain);
	misp_info("BlkOst(R,G,B): (%d, %d, %d).",
		ptMetadata->tPipe0Info.BlackOffset_R,
		ptMetadata->tPipe0Info.BlackOffset_G,
		ptMetadata->tPipe0Info.BlackOffset_B);
	misp_info("Crop(x,y,w,h): (%d, %d, %d, %d).",
		ptMetadata->tPipe0Info.Crop_X,
		ptMetadata->tPipe0Info.Crop_Y,
		ptMetadata->tPipe0Info.Crop_Width,
		ptMetadata->tPipe0Info.Crop_Height);
	misp_info("Scalar(w,h): (%d, %d).",
		ptMetadata->tPipe0Info.ScalarWidth,
		ptMetadata->tPipe0Info.ScalarHeight);
	misp_info("VCM(macro,infinity,curStep,status): (%d, %d, %d, %d).",
		ptMetadata->tPipe0Info.VCM_macro,
		ptMetadata->tPipe0Info.VCM_infinity,
		ptMetadata->tPipe0Info.VCM_CurStep,
		ptMetadata->tPipe0Info.Depth_VCMStatus);

	misp_info("---=== Metadata:PIPE1 Info ===----");
	misp_info("Frame Index: %d.",
		ptMetadata->tPipe1Info.FrameIndex);
	misp_info("SrcImg (width, height, ColorOrder):(%d, %d, %d).",
		ptMetadata->tPipe1Info.SrcImgWidth,
		ptMetadata->tPipe1Info.SrcImgHeight,
		ptMetadata->tPipe1Info.ColorOrder);
	misp_info("(ExpTime, wBV): (%d, %d).",
		ptMetadata->tPipe1Info.ExpTime, ptMetadata->tPipe1Info.BV);
	misp_info("(ISO, ADGain): (%d, %d).",
		ptMetadata->tPipe1Info.ISO, ptMetadata->tPipe1Info.AD_Gain);
	misp_info("AWBGain(R,G,B): (%d, %d, %d).",
		ptMetadata->tPipe1Info.AWB_RGain,
		ptMetadata->tPipe1Info.AWB_GGain,
		ptMetadata->tPipe1Info.AWB_BGain);
	misp_info("BlkOst(R,G,B): (%d, %d, %d).",
		ptMetadata->tPipe1Info.BlackOffset_R,
		ptMetadata->tPipe1Info.BlackOffset_G,
		ptMetadata->tPipe1Info.BlackOffset_B);
	misp_info("Crop(x,y,w,h): (%d, %d, %d, %d).",
		ptMetadata->tPipe1Info.Crop_X,
		ptMetadata->tPipe1Info.Crop_Y,
		ptMetadata->tPipe1Info.Crop_Width,
		ptMetadata->tPipe1Info.Crop_Height);
	misp_info("Scalar(w,h): (%d, %d).",
		ptMetadata->tPipe1Info.ScalarWidth,
		ptMetadata->tPipe1Info.ScalarHeight);
	misp_info("VCM(macro,infinity,curStep,status): (%d, %d, %d, %d).",
		ptMetadata->tPipe1Info.VCM_macro,
		ptMetadata->tPipe1Info.VCM_infinity,
		ptMetadata->tPipe1Info.VCM_CurStep,
		ptMetadata->tPipe1Info.Depth_VCMStatus);

	misp_info("---=== Metadata:Depth Info ===----");
	misp_info("Depth index: %d.",
		ptMetadata->tDpethInfo.DepthIndex);
	misp_info("Reference Frame Index: %d.",
		ptMetadata->tDpethInfo.ReferenceFrameIndex);
	misp_info("Depth(w,h,type) = (%d, %d, %d).",
		ptMetadata->tDpethInfo.DepthWidth,
		ptMetadata->tDpethInfo.DepthHeight,
		ptMetadata->tDpethInfo.DepthType);
	misp_info("VCM Step = (%d, %d).",
		ptMetadata->tDpethInfo.awDepth_VCMStep[0],
	...
	misp_info("ModuleBypass[0~ 5]: 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X.",
		ptMetadata->tSWDebugInfo.aucModuleByPass[0],
		ptMetadata->tSWDebugInfo.aucModuleByPass[1],
		ptMetadata->tSWDebugInfo.aucModuleByPass[2],
		ptMetadata->tSWDebugInfo.aucModuleByPass[3],
		ptMetadata->tSWDebugInfo.aucModuleByPass[4],
		ptMetadata->tSWDebugInfo.aucModuleByPass[5]);
	misp_info("ModuleBypass[6~12]: 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X.",
		ptMetadata->tSWDebugInfo.aucModuleByPass[6],
		ptMetadata->tSWDebugInfo.aucModuleByPass[7],
		ptMetadata->tSWDebugInfo.aucModuleByPass[8],
		ptMetadata->tSWDebugInfo.aucModuleByPass[9],
		ptMetadata->tSWDebugInfo.aucModuleByPass[10],
		ptMetadata->tSWDebugInfo.aucModuleByPass[11]);
	misp_info("DepthInSize: %dx%d.", ptMetadata->
		tSWDebugInfo.DepthInWidth,
		ptMetadata->tSWDebugInfo.DepthInHeight);
	misp_info("Pipe0Shading(w,h,inw,inh):(%d, %d, %d, %d)",
	...
	misp_info("Pipe0Shading(woix,woiy,woiw,woih):(%d, %d, %d, %d)",
	...
	misp_info("Pipe1Shading(w,h,inw,inh):(%d, %d, %d, %d)",
	...
	misp_info("Pipe1Shading(woix,woiy,woiw,woih):(%d, %d, %d, %d)",
	...
	misp_info("SysTime(us): %llu us.", ptMetadata->
		tSWDebugInfo.SysTimeus);
	...
	misp_info("IDD0_ISRTimeus: %llu us.", ptMetadata->
		tSWDebugInfo.IDD0_ISRTimeus);
	misp_info("IDD1_ISRTimeus: %llu us.", ptMetadata->
		tSWDebugInfo.IDD1_ISRTimeus);

#ifdef NEW_ABS
	DualPathFrameSyncTimeDiff =
		abs((s64)(ptMetadata->tSWDebugInfo.IDD1_ISRTimeus -
			ptMetadata->tSWDebugInfo.IDD0_ISRTimeus));
#else
	DualPathFrameSyncTimeDiff =
		abs64((s64)(ptMetadata->tSWDebugInfo.IDD1_ISRTimeus -
			ptMetadata->tSWDebugInfo.IDD0_ISRTimeus));
#endif

	misp_info("DualPathFrameSyncTimeDiff: %llu us.",
		DualPathFrameSyncTimeDiff);

	misp_info("---=== Metadata:IQDebug ===----");
	misp_info("QMerge_Ver: %d.", ptMetadata->tIQDebugInfo.Qmerge_Ver);
	misp_info("Tool_Ver: %d. %d .%d.",
		ptMetadata->tIQDebugInfo.audTool_Ver[0],
		ptMetadata->tIQDebugInfo.audTool_Ver[1],
		ptMetadata->tIQDebugInfo.audTool_Ver[2]);
	misp_info("audTuning_Ver: %d. %d .%d.",
		ptMetadata->tIQDebugInfo.audTuning_Ver[0],
		ptMetadata->tIQDebugInfo.audTuning_Ver[1],
		ptMetadata->tIQDebugInfo.audTuning_Ver[2]);
	misp_info("Verify Debug: %d. %d.",
		ptMetadata->tIQDebugInfo.aucVerifyDebug[0],
	...
	misp_info("EngineEnable: %d. %d. %d. %d. %d. %d. %d. %d.",
		ptMetadata->tIQDebugInfo.aucEngineEnable[0],
		ptMetadata->tIQDebugInfo.aucEngineEnable[1],
		ptMetadata->tIQDebugInfo.aucEngineEnable[2],
		ptMetadata->tIQDebugInfo.aucEngineEnable[3],
		ptMetadata->tIQDebugInfo.aucEngineEnable[4],
		ptMetadata->tIQDebugInfo.aucEngineEnable[5],
		ptMetadata->tIQDebugInfo.aucEngineEnable[6],
		ptMetadata->tIQDebugInfo.aucEngineEnable[7]);
	misp_info("CCM[0]: %d. %d. %d. %d. %d.%d. %d. %d. %d.",
		ptMetadata->tIQDebugInfo.awCCM[0][0],
		ptMetadata->tIQDebugInfo.awCCM[0][1],
		ptMetadata->tIQDebugInfo.awCCM[0][2],
		ptMetadata->tIQDebugInfo.awCCM[0][3],
		ptMetadata->tIQDebugInfo.awCCM[0][4],
		ptMetadata->tIQDebugInfo.awCCM[0][5],
		ptMetadata->tIQDebugInfo.awCCM[0][6],
		ptMetadata->tIQDebugInfo.awCCM[0][7],
		ptMetadata->tIQDebugInfo.awCCM[0][8]);
	misp_info("CCM[1]: %d. %d. %d .%d. %d.%d. %d. %d. %d.",
		ptMetadata->tIQDebugInfo.awCCM[1][0],
		ptMetadata->tIQDebugInfo.awCCM[1][1],
		ptMetadata->tIQDebugInfo.awCCM[1][2],
		ptMetadata->tIQDebugInfo.awCCM[1][3],
		ptMetadata->tIQDebugInfo.awCCM[1][4],
		ptMetadata->tIQDebugInfo.awCCM[1][5],
		ptMetadata->tIQDebugInfo.awCCM[1][6],
		ptMetadata->tIQDebugInfo.awCCM[1][7],
		ptMetadata->tIQDebugInfo.awCCM[1][8]);
	misp_info("ExpRatio: %d.", ptMetadata->tIQDebugInfo.ExpRatio);
	misp_info("ParaAddr[0][ 0~ 4]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][0],
		ptMetadata->tIQDebugInfo.audParaAddr[0][1],
		ptMetadata->tIQDebugInfo.audParaAddr[0][2],
		ptMetadata->tIQDebugInfo.audParaAddr[0][3],
		ptMetadata->tIQDebugInfo.audParaAddr[0][4]);
	misp_info("ParaAddr[0][ 5~ 9]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][5],
		ptMetadata->tIQDebugInfo.audParaAddr[0][6],
		ptMetadata->tIQDebugInfo.audParaAddr[0][7],
		ptMetadata->tIQDebugInfo.audParaAddr[0][8],
		ptMetadata->tIQDebugInfo.audParaAddr[0][9]);
	misp_info("ParaAddr[0][10~14]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][10],
		ptMetadata->tIQDebugInfo.audParaAddr[0][11],
		ptMetadata->tIQDebugInfo.audParaAddr[0][12],
		ptMetadata->tIQDebugInfo.audParaAddr[0][13],
		ptMetadata->tIQDebugInfo.audParaAddr[0][14]);
	misp_info("ParaAddr[0][15~19]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][15],
		ptMetadata->tIQDebugInfo.audParaAddr[0][16],
		ptMetadata->tIQDebugInfo.audParaAddr[0][17],
		ptMetadata->tIQDebugInfo.audParaAddr[0][18],
		ptMetadata->tIQDebugInfo.audParaAddr[0][19]);
	misp_info("ParaAddr[0][20~24]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][20],
		ptMetadata->tIQDebugInfo.audParaAddr[0][21],
		ptMetadata->tIQDebugInfo.audParaAddr[0][22],
		ptMetadata->tIQDebugInfo.audParaAddr[0][23],
		ptMetadata->tIQDebugInfo.audParaAddr[0][24]);
	misp_info("ParaAddr[0][25~29]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][25],
		ptMetadata->tIQDebugInfo.audParaAddr[0][26],
		ptMetadata->tIQDebugInfo.audParaAddr[0][27],
		ptMetadata->tIQDebugInfo.audParaAddr[0][28],
		ptMetadata->tIQDebugInfo.audParaAddr[0][29]);
	misp_info("ParaAddr[0][30~34]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][30],
		ptMetadata->tIQDebugInfo.audParaAddr[0][31],
		ptMetadata->tIQDebugInfo.audParaAddr[0][32],
		ptMetadata->tIQDebugInfo.audParaAddr[0][33],
		ptMetadata->tIQDebugInfo.audParaAddr[0][34]);
	misp_info("ParaAddr[0][35~39]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][35],
		ptMetadata->tIQDebugInfo.audParaAddr[0][36],
		ptMetadata->tIQDebugInfo.audParaAddr[0][37],
		ptMetadata->tIQDebugInfo.audParaAddr[0][38],
		ptMetadata->tIQDebugInfo.audParaAddr[0][39]);
	misp_info("ParaAddr[0][40~44]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][40],
		ptMetadata->tIQDebugInfo.audParaAddr[0][41],
		ptMetadata->tIQDebugInfo.audParaAddr[0][42],
		ptMetadata->tIQDebugInfo.audParaAddr[0][43],
		ptMetadata->tIQDebugInfo.audParaAddr[0][44]);
	misp_info("ParaAddr[0][45~49]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][45],
		ptMetadata->tIQDebugInfo.audParaAddr[0][46],
		ptMetadata->tIQDebugInfo.audParaAddr[0][47],
		ptMetadata->tIQDebugInfo.audParaAddr[0][48],
		ptMetadata->tIQDebugInfo.audParaAddr[0][49]);
	misp_info("ParaAddr[0][50~54]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][50],
		ptMetadata->tIQDebugInfo.audParaAddr[0][51],
		ptMetadata->tIQDebugInfo.audParaAddr[0][52],
		ptMetadata->tIQDebugInfo.audParaAddr[0][53],
		ptMetadata->tIQDebugInfo.audParaAddr[0][54]);
	misp_info("ParaAddr[0][55~59]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][55],
		ptMetadata->tIQDebugInfo.audParaAddr[0][56],
		ptMetadata->tIQDebugInfo.audParaAddr[0][57],
		ptMetadata->tIQDebugInfo.audParaAddr[0][58],
		ptMetadata->tIQDebugInfo.audParaAddr[0][59]);
	misp_info("ParaAddr[0][60~64]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][60],
		ptMetadata->tIQDebugInfo.audParaAddr[0][61],
		ptMetadata->tIQDebugInfo.audParaAddr[0][62],
		ptMetadata->tIQDebugInfo.audParaAddr[0][63],
		ptMetadata->tIQDebugInfo.audParaAddr[0][64]);
	misp_info("ParaAddr[0][65~69]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][65],
		ptMetadata->tIQDebugInfo.audParaAddr[0][66],
		ptMetadata->tIQDebugInfo.audParaAddr[0][67],
		ptMetadata->tIQDebugInfo.audParaAddr[0][68],
		ptMetadata->tIQDebugInfo.audParaAddr[0][69]);
	misp_info("ParaAddr[0][70~74]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][70],
		ptMetadata->tIQDebugInfo.audParaAddr[0][71],
		ptMetadata->tIQDebugInfo.audParaAddr[0][72],
		ptMetadata->tIQDebugInfo.audParaAddr[0][73],
		ptMetadata->tIQDebugInfo.audParaAddr[0][74]);
	misp_info("ParaAddr[0][75~79]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[0][75],
		ptMetadata->tIQDebugInfo.audParaAddr[0][76],
		ptMetadata->tIQDebugInfo.audParaAddr[0][77],
		ptMetadata->tIQDebugInfo.audParaAddr[0][78],
		ptMetadata->tIQDebugInfo.audParaAddr[0][79]);
	misp_info("ParaAddr[1][ 0~ 4]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][0],
		ptMetadata->tIQDebugInfo.audParaAddr[1][1],
		ptMetadata->tIQDebugInfo.audParaAddr[1][2],
		ptMetadata->tIQDebugInfo.audParaAddr[1][3],
		ptMetadata->tIQDebugInfo.audParaAddr[1][4]);
	misp_info("ParaAddr[1][ 5~ 9]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][5],
		ptMetadata->tIQDebugInfo.audParaAddr[1][6],
		ptMetadata->tIQDebugInfo.audParaAddr[1][7],
		ptMetadata->tIQDebugInfo.audParaAddr[1][8],
		ptMetadata->tIQDebugInfo.audParaAddr[1][9]);
	misp_info("ParaAddr[1][10~14]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][10],
		ptMetadata->tIQDebugInfo.audParaAddr[1][11],
		ptMetadata->tIQDebugInfo.audParaAddr[1][12],
		ptMetadata->tIQDebugInfo.audParaAddr[1][13],
		ptMetadata->tIQDebugInfo.audParaAddr[1][14]);
	misp_info("ParaAddr[1][15~19]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][15],
		ptMetadata->tIQDebugInfo.audParaAddr[1][16],
		ptMetadata->tIQDebugInfo.audParaAddr[1][17],
		ptMetadata->tIQDebugInfo.audParaAddr[1][18],
		ptMetadata->tIQDebugInfo.audParaAddr[1][19]);
	misp_info("ParaAddr[1][20~24]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][20],
		ptMetadata->tIQDebugInfo.audParaAddr[1][21],
		ptMetadata->tIQDebugInfo.audParaAddr[1][22],
		ptMetadata->tIQDebugInfo.audParaAddr[1][23],
		ptMetadata->tIQDebugInfo.audParaAddr[1][24]);
	misp_info("ParaAddr[1][25~29]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][25],
		ptMetadata->tIQDebugInfo.audParaAddr[1][26],
		ptMetadata->tIQDebugInfo.audParaAddr[1][27],
		ptMetadata->tIQDebugInfo.audParaAddr[1][28],
		ptMetadata->tIQDebugInfo.audParaAddr[1][29]);
	misp_info("ParaAddr[1][30~34]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][30],
		ptMetadata->tIQDebugInfo.audParaAddr[1][31],
		ptMetadata->tIQDebugInfo.audParaAddr[1][32],
		ptMetadata->tIQDebugInfo.audParaAddr[1][33],
		ptMetadata->tIQDebugInfo.audParaAddr[1][34]);
	misp_info("ParaAddr[1][35~39]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][35],
		ptMetadata->tIQDebugInfo.audParaAddr[1][36],
		ptMetadata->tIQDebugInfo.audParaAddr[1][37],
		ptMetadata->tIQDebugInfo.audParaAddr[1][38],
		ptMetadata->tIQDebugInfo.audParaAddr[1][39]);
	misp_info("ParaAddr[1][41~44]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][40],
		ptMetadata->tIQDebugInfo.audParaAddr[1][41],
		ptMetadata->tIQDebugInfo.audParaAddr[1][42],
		ptMetadata->tIQDebugInfo.audParaAddr[1][43],
		ptMetadata->tIQDebugInfo.audParaAddr[1][44]);
	misp_info("ParaAddr[1][45~49]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][45],
		ptMetadata->tIQDebugInfo.audParaAddr[1][46],
		ptMetadata->tIQDebugInfo.audParaAddr[1][47],
		ptMetadata->tIQDebugInfo.audParaAddr[1][48],
		ptMetadata->tIQDebugInfo.audParaAddr[1][49]);
	misp_info("ParaAddr[1][50~54]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][50],
		ptMetadata->tIQDebugInfo.audParaAddr[1][51],
		ptMetadata->tIQDebugInfo.audParaAddr[1][52],
		ptMetadata->tIQDebugInfo.audParaAddr[1][53],
		ptMetadata->tIQDebugInfo.audParaAddr[1][54]);
	misp_info("ParaAddr[1][55~59]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][55],
		ptMetadata->tIQDebugInfo.audParaAddr[1][56],
		ptMetadata->tIQDebugInfo.audParaAddr[1][57],
		ptMetadata->tIQDebugInfo.audParaAddr[1][58],
		ptMetadata->tIQDebugInfo.audParaAddr[1][59]);
	misp_info("ParaAddr[1][60~64]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][60],
		ptMetadata->tIQDebugInfo.audParaAddr[1][61],
		ptMetadata->tIQDebugInfo.audParaAddr[1][62],
		ptMetadata->tIQDebugInfo.audParaAddr[1][63],
		ptMetadata->tIQDebugInfo.audParaAddr[1][64]);
	misp_info("ParaAddr[1][65~69]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][65],
		ptMetadata->tIQDebugInfo.audParaAddr[1][66],
		ptMetadata->tIQDebugInfo.audParaAddr[1][67],
		ptMetadata->tIQDebugInfo.audParaAddr[1][68],
		ptMetadata->tIQDebugInfo.audParaAddr[1][69]);
	misp_info("ParaAddr[1][70~74]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][70],
		ptMetadata->tIQDebugInfo.audParaAddr[1][71],
		ptMetadata->tIQDebugInfo.audParaAddr[1][72],
		ptMetadata->tIQDebugInfo.audParaAddr[1][73],
		ptMetadata->tIQDebugInfo.audParaAddr[1][74]);
	misp_info("ParaAddr[1][75~79]: 0x%x. 0x%x. 0x%x. 0x%x. 0x%x.",
		ptMetadata->tIQDebugInfo.audParaAddr[1][75],
		ptMetadata->tIQDebugInfo.audParaAddr[1][76],
		ptMetadata->tIQDebugInfo.audParaAddr[1][77],
		ptMetadata->tIQDebugInfo.audParaAddr[1][78],
		ptMetadata->tIQDebugInfo.audParaAddr[1][79]);

	misp_info("---=== Metadata:Histogram ===----");
	misp_info("Hist_S[256~259]: [%d. %d. %d. %d].",
		ptMetadata->audHDRAEHistogram_Short[256],
		ptMetadata->audHDRAEHistogram_Short[257],
		ptMetadata->audHDRAEHistogram_Short[258],
		ptMetadata->audHDRAEHistogram_Short[259]);
	misp_info("Hist_L[256~259]: [%d. %d. %d. %d].",
		ptMetadata->audHDRAEHistogram_Long[256],
		ptMetadata->audHDRAEHistogram_Long[257],
		ptMetadata->audHDRAEHistogram_Long[258],
...
errcode mini_isp_debug_metadata_info(void)
{
	errcode errcode = ERR_SUCCESS;
	u32 metadata_addr;
	struct CAPFLOWMGR_METADATA *ptMetadata = NULL;
	struct misp_global_variable *dev_global_variable;
	...
	mini_isp_memory_read(DEBUG_INFO_METADATA_ADDR,
	(u8 *)&metadata_addr, 4);
	if (metadata_addr == 0) {
	...
	ptMetadata = kzalloc(sizeof(struct CAPFLOWMGR_METADATA), GFP_KERNEL);
	if (ptMetadata == NULL) {
		errcode = -ENOMEM;
	...
	mini_isp_memory_read(metadata_addr, (u8 *)ptMetadata,
		sizeof(struct CAPFLOWMGR_METADATA));
...
static void mini_isp_debug_sensor_info_parser(
			struct FEC_SENSOR_REAL_INFO atRealSensorInfo[],
			u8 sensor_info_num, bool a_IsLog2File)
{
	...
	if (a_IsLog2File) {
		...
		/*fp = filp_open(filename, O_CREAT | O_RDWR, 0777);*/
		...
		for (i = 0; i < sensor_info_num; i++) {
			...
			for (i = 0; i < sensor_info_num; i++) {
				CurLogSize += snprintf(&LogBuffer[CurLogSize],
				128, "---=== Sensor %d ===----\n", i);
				CurLogSize += snprintf(&LogBuffer[CurLogSize],
		...
		for (i = 0; i < sensor_info_num; i++) {
			...
			misp_info("ImageWidth: %d.",
				atRealSensorInfo[i].ImageWidth);
			misp_info("ImageHeight: %d.",
				atRealSensorInfo[i].ImageHeight);
			misp_info("FrameTime: %d us.",
				atRealSensorInfo[i].FrameTime);

			misp_info("FrameRate: %d.%d fps.",
				atRealSensorInfo[i].FrameRate/100,
				atRealSensorInfo[i].FrameRate % 100);
			misp_info("ExpTime: %d us.",
				atRealSensorInfo[i].ExpTime);
			misp_info("Gain: %d.%dx.",
				atRealSensorInfo[i].Gain/100,
				atRealSensorInfo[i].Gain % 100);
			misp_info("Mode: %d. (0~2: normal, master, slave)",
				atRealSensorInfo[i].SensorMode);
		}
	}
...
errcode mini_isp_debug_sensor_info(bool a_IsLog2File)
{
	...
	struct FEC_SENSOR_REAL_INFO atRealSensorInfo[2];
	u8 write_buffer[4];
	u8 sensor_info_num = 1;
	struct misp_global_variable *dev_global_variable;

	dev_global_variable = get_mini_isp_global_variable();
	sensor_info_num = sizeof(atRealSensorInfo) /
		sizeof(struct FEC_SENSOR_REAL_INFO);
	mutex_lock(&dev_global_variable->busy_lock);
	...
	*(u32 *)write_buffer = 1;
	mini_isp_memory_write(DEBUG_INFO_FEC_DEBUG_MODE_ADDR,
		write_buffer, 4);
	msleep(300);

	/* 2. read Sensor Info addr */
	mini_isp_memory_read(DEBUG_INFO_SENSOR_REAL_INFO_ADDR,
		(u8 *)&SensorInfo_addr, 4);
	...
	mini_isp_memory_read(SensorInfo_addr, (u8 *)atRealSensorInfo,
		sizeof(atRealSensorInfo));
...
static void mini_isp_debug_led_info_parser(
				struct PROJECTOR_INFO atLED_INFO[],
				u8 led_info_num, bool a_IsLog2File)
{
	u8 i = 0;
	u32 ret = 0;
	char *LogBuffer = NULL;
	...
	if (a_IsLog2File) {
		...
		snprintf(filename, 128, "%s/led_info_log",
			MINIISP_INFO_DUMPLOCATION);
	#if ENABLE_FILP_OPEN_API
		/*fp = filp_open(filename, O_CREAT | O_RDWR, 0777);*/
		...
		for (i = 0; i < led_info_num; i++) {
			...
			CurLogSize += snprintf(&LogBuffer[CurLogSize], 128,
						...
						atLED_INFO[i].Level,
						atLED_INFO[i].Current,
						atLED_INFO[i].MaxCurrent);
			CurLogSize += snprintf(&LogBuffer[CurLogSize], 128,
						"err: 0x%x, ErrStatus: 0x%x, ErrTemperature: %d\n",
						atLED_INFO[i].errCode,
						atLED_INFO[i].ErrStatus,
						atLED_INFO[i].Temperature);
		}
		...
		for (i = 0; i < led_info_num; i++) {
			...
			misp_info("level: %d, cur: %d(mA), MaxCur: %d(mA)",
				atLED_INFO[i].Level, atLED_INFO[i].Current,
				atLED_INFO[i].MaxCurrent);
			misp_info("err: 0x%x, Err: 0x%x, ErrTemp: %d",
				atLED_INFO[i].errCode, atLED_INFO[i].ErrStatus,
				atLED_INFO[i].Temperature);
		}
	}
...
errcode mini_isp_debug_led_info(bool a_IsLog2File)
{
	...
	u32 led_info_addr = 0;
	u32 i = 0;
	struct PROJECTOR_INFO atLED_INFO[5];
	struct misp_global_variable *dev_global_variable;

	dev_global_variable = get_mini_isp_global_variable();
	LED_INFO_NUM = sizeof(atLED_INFO)/sizeof(struct PROJECTOR_INFO);

	mutex_lock(&dev_global_variable->busy_lock);
	mini_isp_a_to_e();

	/* 1. Set Projector debug mode. AL6100 stop AEC control */
	*(u32 *)write_buffer = ADV_LOG_LIGHTING_PROJECTOR;
	mini_isp_memory_write(DEBUG_INFO_FEC_DEBUG_MODE_ADDR,
		write_buffer, 4);
	...
	for (i = 0; i < maxWaitNum; i++) {
		msleep(100);
		mini_isp_memory_read(DEBUG_INFO_FEC_DEBUG_MODE_ADDR,
			(u8 *)&FEC_DEBUG_FLAG, 4);
		if (FEC_DEBUG_FLAG == 0)
			break;
	...
	*(u32 *)write_buffer = ADV_LOG_LIGHTING_FLOOD;
	mini_isp_memory_write(DEBUG_INFO_FEC_DEBUG_MODE_ADDR,
		write_buffer, 4);
	...
	mini_isp_memory_read(DEBUG_INFO_LED_INFO_ADDR,
		(u8 *)&led_info_addr, 4);

	/* 6. read led info context */
	mini_isp_memory_read(led_info_addr,
		(u8 *)atLED_INFO, sizeof(atLED_INFO));
...
int mipi_rx_fps_body(void *arg)
{
	...
	if (IsLog2File) {
		...
		snprintf(filename, 128, "%s/rx_fps_log",
			MINIISP_INFO_DUMPLOCATION);
	#if ENABLE_FILP_OPEN_API
		/*fp = filp_open(filename, O_CREAT | O_RDWR, 0777);*/
	...
	while (!kthread_should_stop()) {
		...
		TimeNow_ms = (TimeNow.tv_sec * 1000) +
			(TimeNow.tv_usec / 1000);
		TimePrev_ms = (TimePrev.tv_sec * 1000) +
			(TimePrev.tv_usec / 1000);
		Timediff_ms = (u32) (TimeNow_ms - TimePrev_ms);

		/** X1000 for ms precision, X100 for the two
		...
		if (IsLog2File) {
			...
			CurLogSize += snprintf(&LogBuffer[CurLogSize], 128,
					...
					SysTimeNow_sec, Temp_u64,
					Rx0_fps_d, Rx0_fps_f, Rx1_fps_d,
					Rx1_fps_f);
		} else{
			Temp_u64 = SysTimeNow_ns;
			do_div(Temp_u64, 1000);
			misp_info("%s test", __func__);
			misp_info("[%5lld.%06lld]:Rx0fps:%d.%02d, Rx1fps:%d.%02d",
				SysTimeNow_sec, Temp_u64,
...
int mini_isp_debug_mipi_rx_fps_start(bool a_IsLog2File)
{
	...
	if (!mipi_rx_fps_task_active) {
		mipi_rx_fps_task = kthread_create(mipi_rx_fps_body,
			&IsLog2File, "mipi_rx_fps_task");
		if (IS_ERR(mipi_rx_fps_task)) {
			ret = PTR_ERR(mipi_rx_fps_task);
			mipi_rx_fps_task = NULL;
			misp_info("%s, mipi_rx_fps_task initial fail, 0x%x",
				__func__, ret);
			goto EXIT;
		}
...
void mini_isp_debug_mipi_rx_fps_stop(void)
{
	 ...
	 *\Can't delete task if this task is done. otherwise,
	 *\it will busy waiting
	 */
	if (mipi_rx_fps_task_active) {
...
errcode mini_isp_debug_GPIO_Status(bool a_IsLog2File)
{
	...
	if (a_IsLog2File) {
		...
		errcode = mini_isp_create_directory(
			MINIISP_INFO_DUMPLOCATION);

		snprintf(filename, 128, "%s/GPIO_Status_log",
			MINIISP_INFO_DUMPLOCATION);
		...
		for (i = 0; i < GPIO_NUMBER; i++) {
			/* read GPIO status */
			mini_isp_register_read(g_atGPIO[i].addr,
				&aGPIO_RegVal[i]);
