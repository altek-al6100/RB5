...
errcode ispctrl_if_mast_execute_cmd(u16 opcode, u8 *param)
{
	...
	if (opcode >= 0x2000 && opcode <= 0x2FFF) {// bulk cmd
		devdata = (void *)get_mini_isp_intf(MINIISP_BULK_OPCODE);
	} else
...
#else
	/* only get return register value*/
	/* If AP ask exceeds 64byte data size, it will return more than one
	 * package, than here response_buf will be overwrite. Need to write file
	 */
	memcpy(response_buf,
		&rx_buf[ISPCMD_CMDSIZEWDUMMY +
		sizeof(struct system_cmd_isp_register)],
		total_count * 4);
#endif
						...
						u8 *param)
{
	...
	else if (opcode == ISPCMD_SYSTEM_GET_PERIPHERAL_DEVICE)
		err = mast_sys_manage_cmd_get_peripheral_device(devdata,
							opcode, param);
	/*Set PERIPHERAL device*/
	else if (opcode == ISPCMD_SYSTEM_SET_PERIPHERAL_DEVICE)
		err = mast_sys_manage_cmd_set_peripheral_device(devdata,
							opcode, param);
	/*Get Bin file version*/
	else if (opcode == ISPCMD_SYSTEM_GET_BINFILE_VER)
		err = mast_sys_manage_cmd_get_binfile_ver(devdata,
							opcode, param);
	/*Turn on OCRam 7*/
	else if (opcode == ISPCMD_SYSTEM_OCRAM7_TURNON)
		err = mast_sys_manage_cmd_turnon_Ocram7(devdata,
							opcode, param);
	return err;
					...
					u8 *param)
{
	...
	struct transferdata fsm_param = {0};

	fsm = get_mini_isp_fsm();
	/* Set Depth 3A info*/
	if (opcode == ISPCMD_BASIC_SET_DEPTH_3A_INFO) {
		if (IS_EN_FSM) {
			fsm_param.opcode = ISPCMD_BASIC_SET_DEPTH_3A_INFO;
			fsm_param.data = param;
			fsm_param.devdata = devdata;
			err = altek_statefsmispdrv_set_qp(fsm,
				(void *) &fsm_param);
		} else
				...
				u16 opcode, u8 *param)
{
	...
	if (opcode == ISPCMD_BULK_WRITE_BOOTCODE) {
		...
		if (dev_global_variable->bootfromspinor)
			misp_info("%s : boot from SPINor, skip", __func__);
		else{
			/* Write Boot Code*/
			misp_info("%s : write BOOT_CODE", __func__);
			*block_size = MASTERTX_BLOCK_SIZE;
			err = mast_bulk_data_cmd_write_boot_code(devdata,
					param);
		}
		...
		if (dev_global_variable->bootfromspinor)
			...
			if (IS_EN_FSM) {
				fsm_param.opcode = ISPCMD_BULK_WRITE_BASICCODE;
				fsm_param.data = param;
				fsm_param.devdata = devdata;
				err = altek_statefsmispdrv_write_basic_code(fsm,
					(void *) &fsm_param);
			} else
		...
		if (dev_global_variable->bootfromspinor)
			...
			misp_info("%s : write BASIC_CODE", __func__);
			if (IS_EN_FSM) {
			fsm_param.opcode = ISPCMD_BULK_WRITE_BASICCODE_CODESUM;
			fsm_param.data = param;
			fsm_param.devdata = devdata;
			err = altek_statefsmispdrv_write_basic_code(fsm,
				(void *) &fsm_param);
		...
		if (param[8] == CALIBRATION_ID_OTP) {
			misp_info("%s : write IQ_CALIBRATION_DATA", __func__);
		} else if (param[8] == CALIBRATION_ID_DEPTH) {
			misp_info("%s : write DEPTH_CALIBRATION_DATA",
				__func__);
		} else if (param[8] == CALIBRATION_ID_SCID) {
			misp_info("%s : write SCENARIO_CODE", __func__);
		} else if (param[8] == CALIBRATION_ID_HDR) {
			misp_info("%s : write HDR_CODE", __func__);
		} else if (param[8] == CALIBRATION_ID_IRP0) {
			misp_info("%s : write IRP0_CODE", __func__);
		} else if (param[8] == CALIBRATION_ID_IRP1) {
			misp_info("%s : write IRP1_CODE", __func__);
		} else if (param[8] == CALIBRATION_ID_PPMAP) {
			misp_info("%s : write PPMAP_CODE", __func__);
		} else if (param[8] == CALIBRATION_ID_BLENDINGTABLE) {
			misp_info("%s : write BLENDING_TABLE", __func__);
		} else if (param[8] == CALIBRATION_ID_QMERGE) {
			misp_info("%s : write DEPTH_CODE", __func__);
		} else if (param[8] == CALIBRATION_ID_EEPROM) {
			misp_info("%s : write OTP DATA", __func__);
		} else if (param[8] == CALIBRATION_ID_ALTEK) {
			misp_info("%s : write CALIB DATA", __func__);
		} else if (param[8] == CALIBRATION_ID_AE)
			misp_info("%s : write AE Bin", __func__);
	...
	/* Write memory Data*/
	else if (opcode == ISPCMD_BULK_WRITE_MEMORY)
		err = mast_bulk_data_cmd_write_memory_data(devdata, param);
	/* Read memory Data*/
	...
	/* Read Depth RectA, B, Invrect*/
		...
		/*Write SPINOR Data*/
		if (param[8] == SPINOR_ID_BOOT)
			misp_info("%s : write SPINOR BOOT DATA ", __func__);
		else if (param[8] == SPINOR_ID_MAIN)
			misp_info("%s : write SPINOR MAIN DATA", __func__);
		else if (param[8] == SPINOR_ID_SCID)
			misp_info("%s : write SPINOR SC Table", __func__);
		else if (param[8] == SPINOR_ID_HDR)
			misp_info("%s : write SPINOR HDR", __func__);
		else if (param[8] == SPINOR_ID_IRP0)
			misp_info("%s : write SPINOR IRP0", __func__);
		else if (param[8] == SPINOR_ID_IRP1)
			misp_info("%s : write SPINOR IRP1", __func__);
		else if (param[8] == SPINOR_ID_DEPTHBIN)
			misp_info("%s : write SPINOR DEPTH BIN", __func__);
		err = mast_bulk_data_cmd_write_spinor_data(
					devdata, param);
	} else if (opcode == ISPCMD_BULK_GET_BINFILE_VER) {
		mast_bulk_data_cmd_get_binfile_version(devdata, param);
	} else
		misp_err("%s L:%d, opcode not found! 0x%x",
					...
					u8 *param)
{
	...
	if (opcode == ISPCMD_CAMERA_SET_SENSORMODE) {
		if (IS_EN_FSM) {
			fsm_param.opcode = ISPCMD_CAMERA_SET_SENSORMODE;
			fsm_param.data = param;
			fsm_param.devdata = devdata;
			sensor_on_off = param[0];
			if (sensor_on_off == 0)
				err = altek_statefsmispdrv_stop(fsm,
				(void *)&fsm_param);
			else
				err = altek_statefsmispdrv_scenario_chg(fsm,
				(void *)&fsm_param);
		} else
	...
	/*Set CP Mode*/
		...
		if (IS_EN_FSM) {
			fsm_param.opcode = ISPCMD_CAMERA_SET_CP_MODE;
			fsm_param.data = param;
			fsm_param.devdata = devdata;
			err = altek_statefsmispdrv_enter_cp(fsm,
			(void *)&fsm_param);
		} else
	...
	/* Preview stream on*/
		...
		if (IS_EN_FSM) {
			fsm_param.opcode = ISPCMD_CAMERA_PREVIEWSTREAMONOFF;
			fsm_param.data = param;
			fsm_param.devdata = devdata;
			err = altek_statefsmispdrv_strm_on_off(fsm,
				(void *)&fsm_param);
		} else
	...
	/* Set AE statistic*/
		...
		if (IS_EN_FSM) {
			fsm_param.opcode = ISPCMD_CAMERA_SET_AE_STATISTICS;
			fsm_param.data = param;
			fsm_param.devdata = devdata;
			err = altek_statefsmispdrv_set_qp(fsm,
				(void *)&fsm_param);
	...
	/* Set led active delay time */
		...
		err = mast_camera_profile_cmd_set_led_active_delay(
						devdata, opcode, param);
	/* Set ISP smart ae control */
	else if (opcode == ISPCMD_CAMERA_ISPSMARTAECONTROLONOFF)
		err = mast_camera_profile_cmd_isp_smart_ae_control(
						devdata, opcode, param);
	else if (opcode == ISPCMD_CAMERA_SET_GROUP_LED_LEVEL)
		err = mast_camera_profile_cmd_set_group_led_level(
						devdata, opcode, param);
	else if (opcode == ISPCMD_CAMERA_SET_CTRL_LED_SEQUENCE)
		err = mast_camera_profile_cmd_set_ctrl_led_seq(
						devdata, opcode, param);
	else if (opcode == ISPCMD_CAMERA_HOST_CTRL_LED_ONOFF)
		err = mast_camera_profile_cmd_host_ctrl_led_onoff(
						devdata, opcode, param);
	else if (opcode == ISPCMD_CAMERA_EXTRA_SENSOR_SYNC)
		err = mast_camera_profile_cmd_extra_sensor_sync(
						devdata, opcode, param);
	else if (opcode == ISPCMD_CAMERA_SET_DEPTH_TYPE)
		err = mast_camera_profile_cmd_set_depth_type(
						devdata, opcode, param);
	else
